-- tests/04_discard/after.sql
-- 目的: 実行後状態の確認（スクショ用）
-- NOTE: action.sql を実行した同一セッションなら @eid 等が残っている。
--       もし別セッションで after を実行する場合は、下の @run_id を手で入れて検索してください。
-- 例: SET @run_id := '20260107000306';

SET NAMES utf8mb4;

SELECT '========== TEST 04: DISCARD (AFTER) ==========' AS msg;
...
-- B) 別セッション救済：DiscardItem- を探す
SELECT '--- search latest Discard items (manual recovery) ---' AS msg;
SELECT equipment_id, name, status_code, project_id, location_id, manager_id, user_id
FROM equipment
WHERE name LIKE 'DiscardItem-%'
...
-- D) 今回作った project を確認（project_no が P-DISC- で始まる）
SELECT '--- projects created by this test (latest) ---' AS msg;
SELECT id, project_no, name, representative_id, status
FROM projects
WHERE project_no LIKE 'P-DISC-%'
ORDER BY id DESC
LIMIT 10;

-- (optional) discarded は terminal なので更新できないことを確認（エラーが出るのが正しい）
SET @actor_id := @uid_admin;
SET @event_type := 'deploy';
UPDATE equipment SET location_id=@loc WHERE equipment_id=@eid;