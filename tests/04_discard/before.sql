-- tests/04_discard/before.sql
-- 目的: 実行前状態の確認（スクショ用）
-- 実行方法: MySQL端末で USE <DB名>; のあと、この内容を貼り付けて実行

SET NAMES utf8mb4;

SELECT '========== TEST 04: DISCARD (BEFORE) ==========' AS msg;
...
-- 2) admin の存在確認（discard は基本 admin が実行する想定）
SELECT '--- admin (for discard) ---' AS msg;
...
-- 3) equipmentのトリガ存在確認（discardには @actor_id / @event_type が必須）
