...
- 貸出登録では「貸出先（loan_to）」を必須とし、履歴（イベント）に必ず記録する（DB制約で強制）
- 返却：返却日、返却先（在庫へ戻す/学内個人へ割当/設置へ戻す/資金元へ返却 など）を記録する
- 返却登録では「返却先（return_to）」を必須とし、履歴（イベント）に必ず記録する（DB制約で強制）
...
- `discarded` / `returned_to_funder` は終端状態（terminal）とし、原則として以後の更新を不可にする（DB制約で強制）
...
- 状態遷移の最低限ルールを制約で担保する（例：二重貸出の禁止、貸出中の廃棄/資金元返却の禁止）
- 終端状態（`discarded` / `returned_to_funder`）は以後更新できないことを制約で担保する
